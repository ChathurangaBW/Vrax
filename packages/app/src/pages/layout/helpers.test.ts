...
import { type Session } from "@vrax/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("vrax://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("vrax://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("vrax://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("vrax://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("vrax://open-project")).toBeUndefined()
    expect(parseDeepLink("vrax://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "vrax://open-project?directory=/a",
      "vrax://other?directory=/b",
      "vrax://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("vrax://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("vrax://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("vrax://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("vrax://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "vrax://new-session?directory=/a",
      "vrax://open-project?directory=/b",
      "vrax://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __VRAX__: {
        deepLinks: ["vrax://open-project?directory=/a"],
      },
    } as unknown as Window & { __VRAX__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["vrax://open-project?directory=/a"])
