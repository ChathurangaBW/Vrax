...
// Copy settings from the old opencode.settings store into vrax.settings if the
// new store has no data yet. This preserves user preferences across the rename.
function migrateSettingsStore() {
  const newStore = getStore("vrax.settings")
  if (newStore.size > 0) return

  const oldStore = getStore("opencode.settings")
  if (oldStore.size === 0) return

  for (const [key, value] of Object.entries(oldStore.store)) {
    newStore.set(key, value)
  }
  log.log("settings migration: copied opencode.settings → vrax.settings")
}
