...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export VRAX_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export VRAX_CLIENT=desktop",
    `export VRAX_SERVER_USERNAME=${shellEscape(username)}`,
    `export VRAX_SERVER_PASSWORD=${shellEscape(password)}`,
