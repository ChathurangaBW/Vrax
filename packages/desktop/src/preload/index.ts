...
const api: ElectronAPI = {
  ...
  campaignsDefaultRoot: () => ipcRenderer.invoke("campaigns:default-root"),
  campaignsSeedConfig: (workingDir) => ipcRenderer.invoke("campaigns:seed-config", workingDir),
  campaignsScan: (root) => ipcRenderer.invoke("campaigns:scan", root),
  campaignsReadFile: (root, campaign, file) => ipcRenderer.invoke("campaigns:read-file", root, campaign, file),
  campaignsWatch: (root) => ipcRenderer.invoke("campaigns:watch", root),
  campaignsUnwatch: (root) => ipcRenderer.invoke("campaigns:unwatch", root),
  campaignsListFiles: (folderPath) => ipcRenderer.invoke("campaigns:list-files", folderPath),
  onCampaignsChanged: (cb) => {
    const handler = (_: unknown, root: string) => cb(root)
    ipcRenderer.on("campaigns:changed", handler)
    return () => ipcRenderer.removeListener("campaigns:changed", handler)
  },

  openDirectoryPicker: (opts) => ipcRenderer.invoke("open-directory-picker", opts),
  openFilePath: (opts) => ipcRenderer.invoke("open-file-path", opts),
  analyzeBinary: (filePath) => ipcRenderer.invoke("analyze-binary", filePath),
