...
import { isRecord } from "@vrax/tui/util/record"
...
import { TuiKeybind } from "@vrax/tui/config/keybind"
...
import { TuiConfig } from "@vrax/tui/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.vrax`/`.opencode`
  // folders between cwd and home, and VRAX_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.VRAX_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit VRAX_TUI_CONFIG override, if set.
  if (Flag.VRAX_TUI_CONFIG) {
    const configFile = Flag.VRAX_TUI_CONFIG
  ...
  // 4. `.opencode` directories (and VRAX_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".opencode") || dir.endsWith(".vrax") || dir === Flag.VRAX_CONFIG_DIR,
  )

  for (const dir of dirs) {
    if (!dir.endsWith(".opencode") && !dir.endsWith(".vrax") && dir !== Flag.VRAX_CONFIG_DIR) continue
