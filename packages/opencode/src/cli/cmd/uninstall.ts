...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".vrax") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Vrax!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (
      ...
      content.includes("# vrax") ||
      content.includes(".vrax/bin")
    ) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# opencode" || trimmed === "# vrax") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".opencode/bin") || trimmed.includes(".vrax/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && (trimmed.includes(".opencode/bin") || trimmed.includes(".vrax/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".opencode") || trimmed.includes(".vrax")))
    ) {
      continue
    }
