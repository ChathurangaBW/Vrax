...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.VRAX_CONFIG
  delete process.env.VRAX_TUI_CONFIG
...
it.instance("project config takes precedence over VRAX_TUI_CONFIG (matches VRAX_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("VRAX_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from VRAX_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
