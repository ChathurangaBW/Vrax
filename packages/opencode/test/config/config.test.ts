...
import { NamedError } from "@vrax/core/util/error"
...
import { CrossSpawnSpawner } from "@vrax/core/cross-spawn-spawner"
...
import { ProjectV2 } from "@vrax/core/project"
...
const managedConfigDir = process.env.VRAX_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.VRAX_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.VRAX_CONSOLE_TOKEN
  else process.env.VRAX_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})
...
it.effect("does not create global config when VRAX_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:VRAX_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.effect("does not try to install dependencies in read-only VRAX_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("VRAX_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable VRAX_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
// Note: preload.ts sets VRAX_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("VRAX_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { VRAX_CONFIG_DIR: undefined, VRAX_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "VRAX_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { VRAX_DISABLE_PROJECT_CONFIG: "true", VRAX_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed VRAX_PERMISSION JSON used to crash
...
describe("VRAX_PERMISSION env var", () => {
  it.instance("does not crash when VRAX_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "VRAX_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("VRAX_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in VRAX_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
  ...
  it.instance("substitutes {file:} tokens in VRAX_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
