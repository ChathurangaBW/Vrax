...
const original = {
  VRAX_SERVER_PASSWORD: Flag.VRAX_SERVER_PASSWORD,
  VRAX_SERVER_USERNAME: Flag.VRAX_SERVER_USERNAME,
  envPassword: process.env.VRAX_SERVER_PASSWORD,
  envUsername: process.env.VRAX_SERVER_USERNAME,
}
const auth = { username: "opencode", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.VRAX_SERVER_PASSWORD = original.VRAX_SERVER_PASSWORD
  Flag.VRAX_SERVER_USERNAME = original.VRAX_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.VRAX_SERVER_PASSWORD
  else process.env.VRAX_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.VRAX_SERVER_USERNAME
  else process.env.VRAX_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.VRAX_SERVER_PASSWORD = auth.password
  Flag.VRAX_SERVER_USERNAME = auth.username
  process.env.VRAX_SERVER_PASSWORD = auth.password
  process.env.VRAX_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.VRAX_SERVER_PASSWORD = undefined
  Flag.VRAX_SERVER_USERNAME = auth.username
  delete process.env.VRAX_SERVER_PASSWORD
  process.env.VRAX_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
