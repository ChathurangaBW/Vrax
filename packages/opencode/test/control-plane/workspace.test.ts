...
import { EventSequenceTable } from "@vrax/core/event/sql"
...
import { WorkspaceTable } from "@vrax/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@vrax/core/ripgrep"

const originalEnv = {
  VRAX_AUTH_CONTENT: process.env.VRAX_AUTH_CONTENT,
  VRAX_EXPERIMENTAL_WORKSPACES: process.env.VRAX_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.VRAX_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.VRAX_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.VRAX_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.VRAX_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.VRAX_EXPERIMENTAL_WORKSPACES).toBe("true")
