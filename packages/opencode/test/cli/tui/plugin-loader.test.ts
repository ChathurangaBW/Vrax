...
import type { TuiAttentionSoundPack } from "@vrax/plugin/tui"
...
const { allThemes, addTheme } = await import("@vrax/tui/context/theme")
...
`,
  ...
  process.env.VRAX_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.VRAX_PLUGIN_META_FILE!,
    )
