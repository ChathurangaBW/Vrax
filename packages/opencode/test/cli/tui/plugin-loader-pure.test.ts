...
`,
  ...
  const pure = process.env.VRAX_PURE
  const meta = process.env.VRAX_PLUGIN_META_FILE
  process.env.VRAX_PURE = "1"
  process.env.VRAX_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.VRAX_PURE
    } else {
      process.env.VRAX_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.VRAX_PLUGIN_META_FILE
    } else {
      process.env.VRAX_PLUGIN_META_FILE = meta
    }
  }
})
