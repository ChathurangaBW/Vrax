...
import { Catalog } from "@vrax/core/catalog"
import { Connector } from "@vrax/core/connector"
import { Credential } from "@vrax/core/credential"
...
import { PluginV2 } from "@vrax/core/plugin"
import { ModelsDevPlugin } from "@vrax/core/plugin/models-dev"
import { Policy } from "@vrax/core/policy"
...
describe("ModelsDevPlugin", () => {
  it.effect("registers key connectors for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.VRAX_MODELS_PATH,
          disabled: Flag.VRAX_DISABLE_MODELS_FETCH,
        }
        Flag.VRAX_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.VRAX_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.VRAX_MODELS_PATH = previous.path
          Flag.VRAX_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
