...
import { OpencodePlugin } from "@vrax/core/plugin/provider/opencode"
...
describe("OpencodePlugin", () => {
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ VRAX_API_KEY: undefined }, () =>
      Effect.gen(function* () {
  ...
  it.effect("uses VRAX_API_KEY as credentials", () =>
    withEnv({ VRAX_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
        yield* plugin.add(OpencodePlugin)
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ VRAX_API_KEY: undefined, CUSTOM_VRAX_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service
        const catalog = yield* Catalog.Service
        yield* plugin.add(OpencodePlugin)
        const transform = yield* catalog.transform()
        yield* transform((catalog) => {
          const item = provider("opencode", { env: ["CUSTOM_VRAX_API_KEY"] })
