...
const opencodeClient = process.env.VRAX_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) delete process.env.VRAX_CLIENT
  else process.env.VRAX_CLIENT = opencodeClient
})
