...
- **ida-pro-mcp proxy server** (`ida_pro_mcp.server`):
  - Runs as an MCP server for VRAX.
...
- **VRAX / Council agents**:
  - `vrax.json` defines MCP servers + **28 council agents** + **6 slash commands** (`/crash`, `/patch`, `/ctf`, `/malware`, `/zero-day`, `/uac-bypass`).
...
- **ida-pro-mcp proxy server** (`ida_pro_mcp.server`):
  ...
  - Merges the LIEF tools into `tools/list`, so a single MCP id (`ida_pro_mcp`) exposes both IDA and LIEF tools.
- **VRAX / Council agents**:
  - `vrax.json` defines an MCP server:
...
You normally talk only to **`@council-orchestrator`**. It delegates via VRAX `Task` and manages state through `scripts/blackboard.py`.
...
- **IDA online vs offline**:
  ...
  - If IDA is **not** running:
    - `ida_pro_mcp` still starts and at least exposes the LIEF tools (thanks to the `tools/list` fallback), so VRAX/Council can still do static triage from disk.
- **File size guard**:
