...
```
                              USER
                               │
                  vrax run --agent council-orchestrator
                               │
                               ▼
              ┌────────────────────────────────────────┐
...
| Pattern | Purpose |
|---|---|
| `vrax.json` | VRAX agent registry, MCP servers, command bindings |
...
| `prompts/agents/<name>.txt` | Canonical VRAX prompt (loaded via `{file:...}` in `vrax.json`) |
...
Stigmergic blackboard, pheromone decay, trigger predicates, cleanup registry, and consensus protocol ported from [Armur-Ai/Pentest-Swarm-AI](https://github.com/Armur-Ai/Pentest-Swarm-AI) (AGPL-3.0). Architecture pattern adaptation for VRAX — not a Go source fork. AI execution via [VRAX](https://vrax.ai).
