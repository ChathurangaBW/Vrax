...
| File | Purpose |
|---|---|
| `opencode.json` | Task permissions — controls who can delegate to whom |
...
3. **Register in `opencode.json`** under `agent` (example name `security-analyst`):
```json
"security-analyst": {
  "permission": {
    "task": {
...
3. If the new phase agent needs to delegate, update `opencode.json` task permissions.
...
Currently the pipeline is strictly sequential. OpenCode may support async subagent delegation in future — when it does, Phase 1 (mapping) and Phase 2 (vulnerability) could run in parallel since they're both IDA-based.
...
| Circular delegation | Task permissions too permissive | Check `opencode.json` — leaf agents should have `"*": "deny"` |
